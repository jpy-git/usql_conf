...
@pytest.mark.parametrize(
    ["input", "expected"],
    [
        [["pg_test"], "pg://test_user:password123@localhost/test_database?sslmode=disable\n"],
...
def test_main(input, expected, capsys, mocker):
    ...
    mocker.patch("usql_conf.cli.Path.home", return_value=Path(__file__).parent / "mock_home")
    mocker.patch("usql_conf.cli.Path.cwd", return_value=Path(__file__).parent / "mock_cwd")
...
def test_main_home_exist_only(input, expected, capsys, mocker):
    """Connection string returned from `~/.usql_conf` if `./.usql_conf` does not exist."""
    mocker.patch("usql_conf.cli.Path.home", return_value=Path(__file__).parent / "mock_home")
